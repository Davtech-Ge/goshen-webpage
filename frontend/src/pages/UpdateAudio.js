...
const UpdatePost = ({ audio, onClose, fetchData }) => {
  const [data, setData] = useState({
    ...audio,
    title: audio?.title,
    author: audio?.author,
    audioBody: audio?.audioBody,
  })
  ...
  return (
    <div className='flex absolute top-0 left-0 right-0 bottom-0 w-full min-h-[calc(100vh-130px)] '>
                      ...
                      <div>
                        {
                          data?.audioBody[0] ? (
                            ...
                            <button className='px-4 py-2 rounded-lg bg-gray-500 hover:bg-gray-600'>Update Audio</button>
                  </form>
                </div>
               </div>
            </div>
